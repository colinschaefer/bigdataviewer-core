...
public class TransformAwareBufferedImageOverlayRenderer extends
		BufferedImageOverlayRenderer implements TransformAwareRenderTarget {
	...
	@Override
	public synchronized BufferedImage setBufferedImageAndTransform(
			final BufferedImage img, final AffineTransform3D transform) {
	...
	public void addTransformListener(
		...
		synchronized (paintedTransformListeners) {
			...
			paintedTransformListeners.add(
					index < 0 ? 0 : index > s ? s : index, listener);
			listener.transformChanged(paintedTransform);
		}
	}
