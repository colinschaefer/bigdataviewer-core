...
public class BrightnessDialog extends JDialog {
	...
	public BrightnessDialog(final Frame owner,
		...
		setupAssignments
				.setUpdateListener(new SetupAssignments.UpdateListener() {
					@Override
					public void update() {
	...
	public static class ColorsPanel extends JPanel {
		...
		public void recreateContent() {
			...
			for (final ConverterSetup setup : setupAssignments
					.getConverterSetups()) {
				final JButton button = new JButton(new ColorIcon(
						getColor(setup)));
				button.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(final ActionEvent e) {
						colorChooser.setColor(getColor(setup));
						final JDialog d = JColorChooser.createDialog(button,
								"Choose a color", true, colorChooser,
								new ActionListener() {
									@Override
		...
		private static void setColor(final ConverterSetup setup,
				final Color color) {
	...
	public static class MinMaxPanels extends JPanel {
		...
		public void recreateContent() {
			final Dimension sliderSize = (rememberSizes && !minMaxPanels
					.isEmpty()) ? minMaxPanels.get(0).sliders
			...
			for (final MinMaxGroup group : setupAssignments.getMinMaxGroups()) {
				final MinMaxPanel panel = new MinMaxPanel(group,
						setupAssignments, this, rememberSizes);
				if (sliderSize != null)
	...
	/**
	 * A panel containing min/max {@link SliderPanel SliderPanels}, view setup
	 * check-boxes and advanced settings.
	 */
	public static class MinMaxPanel extends JPanel implements
		...
		public MinMaxPanel(final MinMaxGroup group,
			...
			if (rememberSizes && !minMaxPanels.minMaxPanels.isEmpty()) {
				final Dimension dim = minMaxPanels.minMaxPanels.get(0).sliders
						.getSize();
				if (dim.width > 0)
			...
			boxesPanel
					.setLayout(new BoxLayout(boxesPanel, BoxLayout.LINE_AXIS));
				...
				box.addActionListener(new ActionListener() {
					...
					public void actionPerformed(final ActionEvent arg0) {
						if (box.isSelected())
							...
							assignments
									.removeSetupFromGroup(setup, minMaxGroup);
					}
				});
				boxesPanel.add(box);
				boxes.add(box);
			...
			dummy.setModel(new SpinnerNumberModel(minMaxGroup.getRangeMax(),
					minMaxGroup.getFullRangeMin(), minMaxGroup
							.getFullRangeMax(), 1));
			...
			spinnerRangeMin.setModel(new SpinnerNumberModel(minMaxGroup
					.getRangeMin(), Integer.MIN_VALUE, Integer.MAX_VALUE, 1));
			spinnerRangeMin.addChangeListener(new ChangeListener() {
				@Override
				public void stateChanged(final ChangeEvent e) {
					final int value = ((Integer) spinnerRangeMin.getValue())
							.intValue();
			...
			spinnerRangeMin.setBorder(BorderFactory.createEmptyBorder(0, 10,
					10, 10));

			final JSpinner spinnerRangeMax = new JSpinner();
			spinnerRangeMax.setModel(new SpinnerNumberModel(minMaxGroup
					.getRangeMax(), Integer.MIN_VALUE, Integer.MAX_VALUE, 1));
			spinnerRangeMax.addChangeListener(new ChangeListener() {
				@Override
				public void stateChanged(final ChangeEvent e) {
					final int value = ((Integer) spinnerRangeMax.getValue())
							.intValue();
			...
			spinnerRangeMax.setBorder(BorderFactory.createEmptyBorder(0, 10,
					10, 10));

			final JButton advancedButton = new JButton(">>");
			advancedButton.setBorder(BorderFactory.createEmptyBorder(0, 10, 0,
					10));
		...
		public void update() {
			for (int i = 0; i < boxes.size(); ++i) {
				final int setupId = setupAssignments.getConverterSetups()
						.get(i).getSetupId();
				boolean b = false;
